...
contains
  ...
  subroutine sfc_boundary_layer ( dt, Time, Atm, Land, Ice, Land_Ice_Atmos_Boundary )
    ...
    call put_to_xgrid (Atm%hs    , 'ATM', ex_hs    , xmap_sfc, remap_method=remap_method, complete=.false.) !brandon & liao
    ...
    ! s:ie),[5] compute explicit fluxes and tendencies at all available points ---
    ...
    do l = 1, my_nblocks
       ...
       call surface_flux (&
            ...
            ex_wind(is:ie), ex_u10N(is:ie),  ex_u_star(is:ie), ex_b_star(is:ie), ex_q_star(is:ie),                     & ! bgr_prustogi
          ...
          if (atm%fields%bc(n)%use_10m_wind_speed) then  !{
             if (.not. ex_gas_fields_atm%bc(n)%field(ind_u10)%override) then  !{
                do i = is,ie
                   ...
                   ex_gas_fields_atm%bc(n)%field(ind_u10)%values(i) = ex_u10N(i) ! bgr_rustogi # Overwrites U10 with U10N
   ...
   do n = 1, ex_gas_fluxes%num_bcs  !{
      ...
      ! bgr_prustogi
      if (id_tr_out1(m) .gt. 0) then
            call get_from_xgrid (diag_atm, 'ATM', ex_gas_fluxes%bc(n)%field(ind_out1)%values(:), xmap_sfc)
            used = send_data ( id_tr_out1(m), diag_atm, Time )
      end if
      if (id_tr_out2(m) .gt. 0) then
            call get_from_xgrid (diag_atm, 'ATM', ex_gas_fluxes%bc(n)%field(ind_out2)%values(:), xmap_sfc)
            used = send_data ( id_tr_out2(m), diag_atm, Time )
      end if
   end do
...
110 format (f4.1,' m')
    ...
    allocate(id_tr_out1(n_exch_tr)) ! bgr_prustogi
    allocate(id_tr_out2(n_exch_tr))
...
!f1p
       ...
       id_tr_out1(tr) = register_diag_field(mod_name, trim(name)//'_u10', atmos_axes, Time, & ! bgr_prustogi
            'wind speed in parameterization of '//trim(longname), 'm/s')
       id_tr_out2(tr) = register_diag_field(mod_name, trim(name)//'_ust', atmos_axes, Time, &
            'wind friction in parameterization of '//trim(longname), 'm/s')
